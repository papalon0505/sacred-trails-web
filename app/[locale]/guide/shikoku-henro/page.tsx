...
export default async function ShikokuGuide({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params
  const l = locale as Locale
  const content = getGuideContent(SLUG, l)
  const shikokuRoutes = routes.filter(r => r.slug === 'shikoku-henro')
  return <GuideArticle locale={l} content={content} slug={SLUG} featuredRoutes={shikokuRoutes} />
}
